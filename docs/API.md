...
| ~~`PUT /Documentacion/:Matricula`~~ | — | **RETIRADO (7.3 D1-C2)** → 404. `Documentacion` es cache server-computed (recalc en las mutaciones documentales); el cliente nunca la fija. |
...
| ~~`GET /doctosProfile/:id`~~ | **RETIRADO (7.3 D2-C)** → 404 (con o sin token). Usar `GET /users/:idLogin/profile-photo` (metadata) + `GET /files/:idDoctos` (binario). |
| ~~`GET /doctos/:Id`~~ | **RETIRADO (7.3 D2-C)** → 404. Usar `GET /me/documents` + `GET /files/:idDoctos`. |
| ~~`GET /getExpediente/:IdDormi`~~ | **RETIRADO (7.3 D2-C)** → 404. Usar `GET /documents/review/students`. |
| ~~`GET /getArchivos/:Dormitorio/:Nombre?/:Apellidos?/:Matricula?`~~ | **RETIRADO (7.3 D2-C)** → 404. Usar `GET /documents/review/students/:idLogin/documents`. |
...
6. ~~**`DIRECT_FILE_ACCESS_BYPASS`**~~ **CERRADO (7.3 D2-C) ✅**: se eliminó `express.static('public')`;
   `GET /uploads/<archivo>` → `404` (con o sin token). El único acceso a binarios es `GET /files/:idDoctos`
   (Bearer + política documental). `public/uploads` sigue existiendo como **almacenamiento interno**
   (multer escribe / `/files` lee), pero **ya no se expone por HTTP**. `Doctos.Archivo` = localización
   interna ≠ URL pública; el identificador externo del binario es `IdDoctos`.
